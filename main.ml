...
let updater indices p y = update indices weights p y 0.01 ;;
let refresh_loss = 1000000 ;;

(* training *)

let train updater dict_stream = 
	let rec aux updater dict_stream t loss n = match (try Some(Stream.next dict_stream) 
								  with _ -> None) with
	| Some dict -> Hashtbl.remove dict "id"; 
			  ...
			  aux updater dict_stream (t + 1) (loss +. (log_loss p y)) n
			  
	| None -> () in aux updater dict_stream 0 0. (Array.length weights);;	

train updater dict_stream;;
